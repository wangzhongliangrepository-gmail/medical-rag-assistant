...
# 附：双塔 bi-encoder vs cross-encoder（召回快 / 重排准的根因）

> 第 4 节讲过召回用双塔、重排用 cross-encoder，这里把两种结构彻底讲透——这是「为什么分两
> 阶段」最底层的答案，也是面试高频追问点。

## 一、双塔 bi-encoder（召回用）

bi-encoder = "两个编码器"。处理 query 和 document 时，各走一个**独立的编码塔**，互不干扰。
注意两座塔**跑在不同时机**：doc 灌库时（离线）就编码存好，query 查询时（在线）才编码：

```
   离线·灌库时（doc 提前编码存好）        在线·查询时（只编码 query）
   ─────────────────────────────        ──────────────────────────
        document                              query
           │                                    │
           ▼                                    ▼
 ...
 │  Qdrant：5万+ doc 向量 │                      │
 │  预存 + 建 HNSW 索引   │◀─────────────────────┘
 │ (近似最近邻图结构)     │   拿 query 向量进 HNSW 图
 └──────────┬──────────┘   "跳着找"最近的~20个（不挨个比）
            ▼
   只对命中的 ~20 个候选算余弦相似度
            ▼
      top-k 候选（dense 路召回结果）
```

**核心特征**：query 和 doc 各自**独立**编码成一个向量，两座塔编码时**没有任何交互**。doc 那座
塔在**灌库时就跑完**、向量存进 Qdrant 并建 HNSW 索引；query 那座塔在**查询时**才跑，拿到 query
向量后**靠 HNSW 跳着找**最近的 ~20 个候选（不是和 5 万个挨个算余弦），只对这少数候选算余弦排序。

> 三层别混在一起：**bi-encoder** 决定"向量怎么来"（分开编码、可预存）、**HNSW** 决定"怎么不挨个
> 比就找到最近的"、**余弦** 决定"近怎么算"——三层叠起来才是 dense 召回全貌。

**为什么快**：正因为分开编码，doc 向量可以**提前算好存库**——
- 5 万+ 段教材的向量在 `ingest.py` 灌库时就用 `embed_documents` 全算好存进 Qdrant；
- 查询时**只算 1 个 query 向量**（`embed_query`），拿去和库里现成的 5 万向量比；
- 配合 HNSW 索引跳着找，秒级返回。

> 一句话：**doc 向量能预存，是双塔召回快的根本原因。**

**为什么粗**：query 和 doc 各压成一个向量后信息已损失，只能比"整体语义像不像"。例：

```
query = "二甲双胍的副作用"
候选A = "二甲双胍常见副作用有恶心、腹泻"     ← 真正想要的
候选B = "二甲双胍的作用机制是抑制肝糖输出"   ← 含"二甲双胍"，整体语义也近
```
双塔可能给 A、B 都打高分（都和"二甲双胍"语义近），**分不清"既要二甲双胍、又要副作用"的组合要求**。

## 二、cross-encoder（重排用）

cross = "交叉"：query 和候选文档**拼成一对、一起喂进同一个模型**，让模型把两者每个词交叉对照：

```
   query「二甲双胍的副作用」      候选「二甲双胍常见副作用有恶心、腹泻」
              └──────────┬──────────┘
                         ▼
   拼成一个输入：[CLS] 二甲双胍的副作用 [SEP] 二甲双胍常见副作用有恶心… [SEP]
                         ▼
              ┌──────────────────────┐
              │   一个 Transformer     │  ← query 每个词能"看到"doc 每个词
              │ (bge-reranker-v2-m3)  │     （注意力做 word-level 交互）
              └──────────┬───────────┘
                         ▼
              直接输出一个 relevance_score（如 0.95）
```

**为什么准**：注意力机制让 query 的"副作用"直接去和 doc 每个词对齐，能判断 doc 到底在不在讲
副作用、"二甲双胍"和"副作用"是不是**同时**满足。对上面例子：

| 候选 | 内容 | 双塔召回 | cross-encoder 重排 |
|------|------|---------|-------------------|
| 候选1 | 二甲双胍常见**副作用**有恶心、腹泻 | 排得还行 | **0.95** ✓ 两点都命中 |
| 候选2 | 二甲双胍的**作用机制**是抑制肝糖输出 | 含"二甲双胍"排不低 | **0.30** 讲机制不是副作用 |
| 候选3 | **格列美脲**的副作用有头晕 | 含"副作用"排不低 | **0.20** 药名不对 |

**为什么慢**：输入是 `[query + 某条 doc]` 拼起来的整体，query 每次都不同 → **没法预存**，20 个候
选就得把模型完整跑 20 次。对 5 万段全跑根本扛不住——所以只用于召回已缩到 20 条之后。

## 三、对比表

| | 双塔 bi-encoder（召回） | cross-encoder（重排） |
|--|--|--|
| query 和 doc | **分开**各自编码成向量 | **拼一起**进模型 |
| 看到交互吗 | 看不到（只比最终向量） | 看得到（逐词对应） |
| doc 能否预存 | ✅ 能 → **快** | ❌ 每条现算 → **慢** |
| 准度 | 粗 | 准 |
| 用在哪 | 召回：5 万 → 20 条 | 重排：20 → 3 条 |

> 类比：**双塔是"各自拍证件照再比脸"，cross-encoder 是"两人面对面谈一次再下结论"**——后者准
> 得多，但没法提前拍好存着，只能现场谈，所以贵。

## 四、对照代码（rerank 的 index 映射坑）

调用层（`embeddings.py` 的 `rerank`，无现成 LangChain 封装，直接打 REST）：
```python
def rerank(query, documents, top_n):
    payload = {"model": RERANK_MODEL_UID, "query": query, "documents": documents}
    resp = requests.post(f"{XINFERENCE_URL}/v1/rerank", json=payload)
    return resp.json()["results"]   # [{index, relevance_score}, ...]  ← 只回下标，不回原文！
```
使用层（`kb_search.py` 的 `search`）：
```python
def search(query, recall=RECALL_K, top_k=RERANK_TOP_K):
    cands = hybrid_recall(query, recall)              # ① 双塔+BM25 召回 20 条（带元数据）
    docs = [c["text"] for c in cands]                 # ② 只抽文本喂 reranker
    ranked = rerank(query, docs, top_n=top_k)         # ③ cross-encoder 精排取 top-3
    return [{**cands[r["index"]], "score": r["relevance_score"]}  # ④ 用 index 映射回原候选
            for r in ranked]
```
- **④ 是关键坑**：reranker 为省带宽**只返回下标 `index` 不返回原文**，必须用 `cands[r["index"]]`
  把 source_id / chunk_id 等元数据接回来（作答标 `[编号]` 引用要用），否则重排完只剩文本、丢了来源。

## 五、面试问答补充

1. 双塔为什么快？→ doc 向量可预存，查询只算 1 个 query 向量，配 HNSW 索引。
2. cross-encoder 为什么准又为什么慢？→ query+doc 拼一起做词级交互所以准；没法预存、每条现算所以慢。
3. 为什么不直接全用 cross-encoder？→ 对 5 万段要跑 5 万次模型扛不住；先双塔召回缩到 20 再精排。
4. rerank 接口返回什么？怎么拿回元数据？→ 返回 `{index, relevance_score}`，用 index 映射回召回候选。
